...
export class CompetitionAnalysisResolver {
  constructor(
    ...
    private readonly viewSearchUsecase: ViewSearchUsecase,
  ...
  async itemSearch(
    ...
    return this.viewSearchUsecase.execute(input, user.code);
  }
  ...
  async searchKeyword(@Args('keyword') keyword: string) {
    return this.competitionAnalysisService.findSearchKeyword(keyword);
  }
